...
def get_nearest_pharmacies(lat, lon):
    API_KEY = '5ff9b945-0f5c-4355-8c90-fe93681fea26'
    URL = f"https://search-maps.yandex.ru/v1/?text=Аптека&type=biz&lang=ru_RU&ll={lon}, {lat}&results=6&spn=0.552069,0.400552&rspn=1&format=json&apikey={API_KEY}"
    result = requests.get(URL).json()
    logger.debug(result)
...
def location():
    if request.method == 'POST':
        ...
        near_pharamacies = get_nearest_pharmacies(lat, lon)
        logger.debug(near_pharamacies)
        return jsonify({'name': geolocation,
            'name' : user.name,
